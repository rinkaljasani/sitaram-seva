<div class="aside aside-left aside-fixed d-flex flex-column flex-row-auto" id="kt_aside">
	...
	<div class="brand flex-column-auto" id="kt_brand">
		...
		<a href="{{ route('admin.dashboard.index') }}" class="brand-logo">
			<img style="max-width: 120px; max-height: 65px;" alt="{{ env('APP_NAME') }}" src="{{ asset($sitesetting['site_logo'] ?? "") }}" />
		</a>
		<!--end::Logo-->
		<!--begin::Toggle-->
			...
			<ul class="menu-nav">
				...
				@foreach ($menu as $section)
					...
					@else
						...
						<li class="menu-item side-item {{ !isset($menu) ?: $menu['class'] }}" 
							aria-haspopup="true" data-title="'.{{strtolower( !isset($menu) ?: $menu['title'] )}}.'">
							<a href="{{ (!empty(!isset($menu) ?: $menu['params'])) ? route(!isset($menu) ?: $menu['route'], 
								!isset($menu) ?: $menu['params']) : route(!isset($menu) ?: $menu['route']) }}" class="menu-link">
								@if( !isset($menu) ?: $menu['icon_type'] == 'image' )
                                	<img class="menu-icon" alt="{{ str_slug(!isset($menu) ?: $menu['title']) }}">
                            	@else
									<i class="menu-icon {{ !isset($menu) ?: $menu['icon'] }}"></i>
								@endif
								<span class="menu-text">{{ !isset($menu) ?: $menu['title'] }}</span>
							</a>
						</li>
					@endif
				@endforeach
