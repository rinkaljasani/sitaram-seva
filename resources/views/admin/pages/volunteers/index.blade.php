...
@push('breadcrumb')
    {!! Breadcrumbs::render('volunteers_list') !!}
@endpush
        ...
        <div class="card-header">
            ...
            <div class="card-toolbar">
                @if (in_array('delete', $permissions))
                    <a href="{{ route('admin.volunteers.destroy', 0) }}" name="del_select" id="del_select" class="btn btn-sm btn-light-danger font-weight-bolder text-uppercase mr-2 delete_all_link">
                        <i class="far fa-trash-alt"></i> Delete Selected
                    </a>
                @endif
                @if (in_array('add', $permissions))
                    <a href="{{ route('admin.volunteers.create') }}" class="btn btn-sm btn-primary font-weight-bolder text-uppercase">
...
<script>
    ...
    function tableData(type){
        oTable = $('#users_table').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: "{{ route('admin.volunteers.listing') }}",
                data: {
                    columnsDef: ['checkbox','name', 'email',  'contact_no','address','image','status',  'active', 'action'],
                    type:type,
                },
            },
            columns: [
            ...
            columnDefs: [
                ...
                { targets: 4, title: 'Address', orderable: true },
                { targets: 5, title: 'Image', orderable: true },
