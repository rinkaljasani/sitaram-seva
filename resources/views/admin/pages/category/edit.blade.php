...
@push('breadcrumb')
    {!! Breadcrumbs::render('users_update', $category->id) !!}
@endpush
...
<div class="container">
    <div class="card card-custom">
        ...
        <!--begin::Form-->
        <form id="frmEditUser" method="POST" action="{{ route('admin.categories.update', $category->custom_id) }}" enctype="multipart/form-data">
            @csrf
            @method('put')
            <div class="card-body">
                {{-- name --}}
                <div class="form-group">
                    <label for="name">Office name{!!$mend_sign!!}</label>
                    <input name="name" type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" value="{{ old('name') != null ? old('name') : $category->name }}" placeholder="Enter name" autocomplete="name" autofocus />
                    @if ($errors->has('name'))
                ...
                <div class="symbol symbol-120 mr-5">
                        <div class="symbol-label" style="background-image:url({{ generateURL($category->image)}})">
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description" placeholder="Enter description" autocomplete="description" spellcheck="true">{{  $category->description }}</textarea>
