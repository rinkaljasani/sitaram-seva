...
<div class="container">
    <div class="card card-custom gutter-b example example-compact">
        ...
        <form id="frmAddRoles" class="form-horizontal" role="form" method="POST" action="{{ route('admin.roles.store') }}" enctype="multipart/form-data">
            @csrf
            <div class="card-body">
                ...
                <div class="form-group{{ $errors->has('contact_no') ? ' has-error' : '' }}">
                    ...
                    <input type="text" name="contact_no" class="form-control" placeholder="Enter your Contact" value="{{ old('contact_no') }}">
                                ...
                                <tr class="role_row_{{ $role->id }}">
                                    ...
                                    <td>
                                        @if ( in_array('access', $allowed_permissions) )
                                            <input class="premision_check role_{{ $role->id }}" data-id="{{ $role->id }}" data-error-container='#permission_error' name="roles[{{ strtolower($role->id) }}][permissions][]" type="checkbox" value="access" {{ $role->id == 1 ? 'checked disabled' : '' }}>
                                            @php $total_count++ @endphp
                                        @else N / A @endif
                                    </td>
                                    <td>
                                        @if ( in_array('add', $allowed_permissions) )
                                            <input class="premision_check role_{{ $role->id }}" data-id="{{ $role->id }}" data-error-container='#permission_error' name="roles[{{ strtolower($role->id) }}][permissions][]" type="checkbox" value="add">
                                            @php $total_count++ @endphp
                                        @else N / A @endif
                                    </td>
                                    <td>
                                        @if ( in_array('edit', $allowed_permissions) )
                                            <input class="premision_check role_{{ $role->id }}" data-id="{{ $role->id }}" data-error-container='#permission_error' name="roles[{{ strtolower($role->id) }}][permissions][]" type="checkbox" value="edit">
                                            @php $total_count++ @endphp
                                        @else N / A @endif
                                    </td>
                                    <td>
                                        @if ( in_array('delete', $allowed_permissions) )
                                            <input class="premision_check role_{{ $role->id }}" data-id="{{ $role->id }}" data-error-container='#permission_error' name="roles[{{ strtolower($role->id) }}][permissions][]" type="checkbox" value="delete">
                                            @php $total_count++ @endphp
                                        @else N / A @endif
                                    </td>
                                    <td>
                                        @if ( in_array('view', $allowed_permissions) )
                                            <input class="premision_check role_{{ $role->id }}" data-id="{{ $role->id }}" data-error-container='#permission_error' name="roles[{{ strtolower($role->id) }}][permissions][]" type="checkbox" value="view">
                                            @php $total_count++ @endphp
                                        @else N / A @endif
                                    </td>
                                    <td>
                                        <input type="checkbox" class="checkrow all_rolechecked_{{ $role->id }}"  name="checkrow" data-roleid='{{ $role->id }}'>
                                    </td>
                                </tr>
                                @endif
                            @endforeach
